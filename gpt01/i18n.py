...
_STRINGS: dict[str, tuple[str, str]] = {
    ...
    "status_new_document": ("New document", "Новый документ"),
    "status_file": ("File: {name}", "Файл: {name}"),
    "status_package": ("Package: {name}", "Пакет: {name}"),
    "status_modified": ("modified", "изменён"),
    "status_offline": ("Internet: not required", "Интернет: не требуется"),
